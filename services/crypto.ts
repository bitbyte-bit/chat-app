...
export const deriveKeyFromPassword = async (password: string, saltStr: string = 'zenj-default-salt') => {
  // Defensive check for Secure Context (HTTPS or localhost)
  if (!window.crypto || !window.crypto.subtle) {
    console.warn("Cryptography unavailable: Insecure context detected. Zenj operates with restricted encryption.");
    return null;
  }
  ...
  masterKey = await window.crypto.subtle.deriveKey(
    {
      ...
      iterations: 25000, // Reduced from 100000 for better performance
      hash: 'SHA-256',
    },
    passwordKey,
...
export const encryptContent = async (text: string): Promise<string> => {
  if (!masterKey || !window.crypto.subtle) return text;
...
export const decryptContent = async (encryptedData: string): Promise<string> => {
  if (!masterKey || !window.crypto.subtle || !encryptedData.startsWith('zenj_v1:')) return encryptedData;
  ...
  try {
    ...
    return ''; // Return empty string instead of showing encrypted message indicator
  }
};
