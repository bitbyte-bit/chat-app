...
import { Camera, ArrowRight, Sparkles, Check, User, Phone, Info, Mail, Lock, Eye, EyeOff } from 'lucide-react';
...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const isIdentityComplete = formData.name && formData.phone && formData.email && formData.password.length >= 6;
        ...
        {step === 2 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right duration-500">
            ...
            <button 
              ...
              className="w-full bg-[#00a884] text-black font-bold py-5 rounded-[24px] flex items-center justify-center gap-2 shadow-2xl shadow-[#00a884]/20 hover:bg-[#06cf9c] transition-all text-lg"
            >
              Initialize Presence <Check size={20} strokeWidth={3} />
            </button>
            <button onClick={() => setStep(1)} className="w-full text-[#8696a0] font-medium text-sm">Back to Identity</button>
          </div>
        )}
